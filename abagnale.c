...
struct abag_tls {
  ...
  struct position_pricing_vars {
    ...
    struct Numeric *restrict r4;
    struct Numeric *restrict r5;
  } position_pricing;
...
static struct abag_tls *abag_tls(void) {
  ...
  if (tls == NULL) {
    ...
    tls->position_pricing.b_inc = Numeric_new();
    ...
    tls->position_pricing.r4 = Numeric_new();
    tls->position_pricing.r5 = Numeric_new();
...
static void abag_tls_dtor(void *e) {
  ...
  Numeric_delete(tls->position_pricing.b_inc);
  ...
  Numeric_delete(tls->position_pricing.r4);
  Numeric_delete(tls->position_pricing.r5);
...
static void position_pricing(const struct worker_ctx *restrict const w_ctx,
                             ...
                             const bool create) {
  const struct abag_tls *restrict const tls = abag_tls();
  struct Numeric *restrict const b_inc = tls->position_pricing.b_inc;
  ...
  struct Numeric *restrict const r4 = tls->position_pricing.r2;
  struct Numeric *restrict const r5 = tls->position_pricing.r2;
  ...
  scale_to_increment(b_inc, t->b_sc);
  ...
  if (create) {
    ...
    if (Numeric_cmp(p->b_ordered, b_inc) < 0) {
      Numeric_add_to(p->b_ordered, b_inc, r0);
  ...
  // r0 = 2 * fee - 1
  Numeric_mul_to(p->b_ordered, r0, r1);
  // r1 = b * (2 * fee - 1)
  Numeric_mul_to(p->b_ordered, p->price, r2);
  // r2 = b * p
  Numeric_sub_to(r2, t->tp, r3);
  // r3 = b * p - tp
  Numeric_mul_to(p->price, r0, r4);
  // r4 = p * (2 * fee - 1)
  Numeric_div_to(t->tp, p->b_ordered, r5);
  // r5 = tp / b
  ...
  case POSITION_TYPE_LONG:
    ...
    Numeric_add_to(r4, r5, p->tp_price);
  ...
  case POSITION_TYPE_SHORT:
    ...
    Numeric_scale(p->sl_price, t->p_sc);
    Numeric_div_to(r3, r1, p->tp_price);
    Numeric_scale(p->tp_price, t->p_sc);
...
static void position_timeout(const struct worker_ctx *restrict const w_ctx,
  ...
  if (w_ctx->p_cnf->sl_dlnanos != NULL) {
...
static void trade_bet(const struct worker_ctx *restrict const w_ctx,
  ...
  struct Position *restrict const p =
      t->a->position_open(w_ctx->db, w_ctx->ex, t, samples, sample);
    ...
    if (!t->open_trg.set) {
      ...
      if (verbose)
        wout("%s: %s->%s: Entering open(%" PRIuMAX ")\n",
             String_chars(w_ctx->ex->nm), String_chars(t->q_id),
             String_chars(t->b_id), t->open_trg.cnt);
    }
